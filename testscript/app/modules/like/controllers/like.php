...
class like extends MX_Controller {
	...
	public function ajax_save_schedule($ids){
		...
		switch (post('speed')) {
			case 'auto':
				$speed = getOption('like_speed_auto', 5);
				break;
			case 'slow':
				$speed = getOption('like_speed_slow', 3);
				break;
			case 'medium':
				$speed = getOption('like_speed_medium', 4);
				break;
			case 'fast':
				$speed = getOption('like_speed_fast', 8);
				break;
		}
	...
	private function action($item){
		...
		switch ($target) {
			case 'tag':
				...
				$feeds = $this->remove_favorited_feed($feeds);
