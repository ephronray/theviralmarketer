...
<div class="pd-20">
  <div class="row automation">
    ...
    <div class="target-setting">
      ...
      <div class="form-group  pt-10">
        ...
        <label class="custom-control custom-radio inline-block item">
          ...
          <label class="custom-control-label" for="customRadio2"><?=lang("Username")?></label>
        </label>
      </div>
    </div>
                    
    <div class="form-group target-setting target-tag <?=((!empty($act) && get_value($act, 'target') == 'username'))?'d-none':''?>">
    ...
    <div class="form-group target-setting target-username <?=($default ||(!empty($act) && get_value($act, 'target') == 'tag'))?'d-none':''?>">
      <span class="small-text"><i class="icon-copy ion-person"></i> <?=lang("Username")?> <?=lang("Target_followers_of_popular_Twitter_accounts_by_listing_their_account_names")?></span>
      <div class="pt-10">
        ...
        <div class="col-md-6">
          <div class="form-group">
            ...
            <select class="form-control" name="speed" data-style="btn-outline-info">
              <option <?=(!empty($act) && get_value($act, "speed")=="auto")?"selected":""?> value="auto"><?=lang('auto')?> <?=sprintf(lang('x_requests_per_hour'),getOption('follow_speed_auto', 5))?></option>
              <option <?=(!empty($act) && get_value($act, "speed")=="slow")?"selected":""?> value="slow"><?=lang('slow')?> <?=sprintf(lang('x_requests_per_hour'),getOption('follow_speed_slow', 3))?></option>
              <option <?=(!empty($act) && get_value($act, "speed")=="medium")?"selected":""?> value="medium"><?=lang('medium')?> <?=sprintf(lang('x_requests_per_hour'),getOption('follow_speed_medium', 4))?></option>
              <option  <?=(!empty($act) && get_value($act, "speed")=="fast")?"selected":""?> value="fast"><?=lang('fast')?> <?=sprintf(lang('x_requests_per_hour'),getOption('follow_speed_fast', 8))?></option>
            </select>
          </div>
        </div>
