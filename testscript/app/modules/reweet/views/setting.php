...
<div class="pd-20">
  <div class="row automation">
    ...
    <div class="form-group target-setting target-username <?=($default ||(!empty($act) && get_value($act, 'target') == 'tag'))?'d-none':''?>">
      <span class="small-text"><i class="icon-copy ion-person"></i> <?=lang("Username")?> <?=lang("enter_some_popular_twitter_accounts_who_you_want_to_like_theirs_newest_tweets")?></span>
      <div class="pt-10">
        ...
        <div class="col-md-6">
          <div class="form-group">
            ...
            <select class="form-control" name="speed" data-style="btn-outline-info">
              <option <?=(!empty($act) && get_value($act, "speed")=="auto")?"selected":""?> value="auto"><?=lang('auto')?> <?=sprintf(lang('x_requests_per_day'),getOption('reweet_speed_auto', 5))?></option>
              <option <?=(!empty($act) && get_value($act, "speed")=="slow")?"selected":""?> value="slow"><?=lang('slow')?> <?=sprintf(lang('x_requests_per_day'),getOption('reweet_speed_slow', 3))?></option>
              <option <?=(!empty($act) && get_value($act, "speed")=="medium")?"selected":""?> value="medium"><?=lang('medium')?> <?=sprintf(lang('x_requests_per_day'),getOption('reweet_speed_medium', 4))?></option>
              <option  <?=(!empty($act) && get_value($act, "speed")=="fast")?"selected":""?> value="fast"><?=lang('fast')?> <?=sprintf(lang('x_requests_per_day'),getOption('reweet_speed_fast', 8))?></option>
            </select>
          </div>
        </div>
