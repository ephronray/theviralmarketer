...
<html lang="en">
  <head>
    ...
    <title>Install BitcoinWallet PHP Script v1.0</title>
  ...
  <body>
	...
	<div class="container"">
		<div class="row">
				    ...
				    <div class="navbar navbar-default rounded-top" role="navigation">
					  <div class="container">
						<div class="navbar-header">
						  <a class="navbar-brand" href="./">BitcoinWallet v1.0</a>
					    </div>
					  </div>
					</div>	
			...
			$api_address = protect($_POST['api_address']);
								...
								if(empty($mysql_host) or empty($mysql_user) or empty($mysql_pass) or empty($mysql_base) or empty($title) or empty($description) or empty($keywords) or empty($api_address) or empty($name) or empty($url) or empty($infoemail) or empty($supportemail) or empty($withdrawal_comission) or empty($api_key) or empty($api_secret_pin)) {
	...
	';
											...
											$update = $db->query("UPDATE btc_settings SET title='$title',description='$description',keywords='$keywords',name='$name',url='$url',infoemail='$infoemail',supportemail='$supportemail',withdrawal_comission='$withdrawal_comission',max_addresses_per_account='10'");
											$passwd = md5($password);
